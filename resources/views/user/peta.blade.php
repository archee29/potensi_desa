        ...
        @foreach ($pasar as $item)
            L.marker([{{ $item->location }}])
                .bindPopup(
                    ...
                    "<div><a href='{{ route('peta.show', $item->slug) }}' class='btn btn-outline-info btn-sm'>Detail Space</a></div>" +
                    "<div class='my-2'></div>"
                ).addTo(map);
        @endforeach
            ...
            @foreach ($pasar as $item)
                L.marker([{{ $item->location }}])
                    .bindPopup(
                        ...
                        "<a href='{{ route('peta.show', $item->slug) }}' class='btn btn-outline-info btn-sm'>Detail Spot</a></div>" +
                        "<div class='my-2'></div>"
                    ).addTo(map);
            @endforeach
        }
